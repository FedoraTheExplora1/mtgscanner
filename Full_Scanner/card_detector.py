...
class CardDetector:
    ...
    def detect_card(self):
        cap = cv2.VideoCapture(2)  # Use the camera with index 0, adjust as needed
        ret, frame = cap.read()  
        if ret:
            ...
            for contour in contours:
                ...
                if len(approx) == 4:
                    # Sort the corners based on y-coordinates
                    sorted_corners = sorted(approx, key=lambda x: x[0][1])

                    # Split the sorted list into top and bottom points
